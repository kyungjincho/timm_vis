...
from timm_vis.timm_vis.helpers import *
import torch.nn as nn

def visualize_filters(
    model, filter_name=None, max_filters=64, size=128, figsize=(16, 16), save_path=None
):
    """
    ...
    w_size = weights.size()
    merged_weights = (
        weights.reshape(w_size[0] * w_size[1], w_size[2],
                        w_size[2]).detach().numpy()
    )
...
def visualize_activations(
    model, module, img_path, max_acts=64, rgb=True, figsize=(16, 16), save_path=None
):
    """
...
def maximally_activated_patches(
    model,
    img_path,
    patch_size=448,
    stride=100,
    ...
    fig, axs = plt.subplots(int(num_patches / 5),
                            min(num_patches, 5), figsize=figsize)
    ...
    for i, idx in enumerate(top_indices):
        ...
        if rgb:
            ...
            axs[plot_idx].imshow(
                np.asarray(img_copy.crop((x0, y0, x1, y1))), cmap="gray"
            )
        axs[plot_idx].set_yticks([])
        axs[plot_idx].set_xticks([])
...
def saliency_map(
    model, img_path, rgb=True, figsize=(16, 16), device="cuda", save_path=None
):
    """
...
def generate_image(
    ...
    for i in range(1, epochs + 1):
        ...
        if i % p_freq == 0 or i == epochs:
            print(
                "Epoch: {} Confidence score for class {}: {}".format(
                    i, target_class, p)
            )

        if p > min_prob:
            print(
                "Reached {} confidence score in epoch {}. Stopping early.".format(
                    p, i)
            )
            break
...
def feature_inversion(
    ...
    for module in modules:
        recreated_imgs.append(
            feature_inversion_helper(
                model,
                module,
                orig_img,
                epochs=epochs,
    ...
    for i in range(len(recreated_imgs)):
        if rgb:
            ...
            axs[i].imshow(postprocess_image(
                recreated_imgs[i], rgb=rgb), cmap="gray")

    if save_path:
        fig.savefig(save_path)


def feature_inversion_helper(
    model,
    module,
    orig_img,
    epochs,
    ...
    for i in range(epochs):
        ...
        loss = ((orig_features - curr) ** 2).sum() + mu * total_variation_regularizer(
            noise
        )
...
def grad_cam(
    ...
    arr = transforms.Resize(
        (img_t.shape[-1], img_t.shape[-2]))(gradcam.unsqueeze(0))
    gradcam_img = arr.detach().cpu().permute((1, 2, 0)).squeeze(-1)

    img = np.asarray(img_t.cpu().numpy()[0, 0, :, :])
    return img, gradcam_img
...
def grad_cam_act(
    ...
    for i in range((class_id)):
        outs[0, i].backward(retain_graph=True)
        gap = torch.mean(
            grads[0][0].view(grads[0][0].size(0), grads[0][0].size(1), -1), dim=2
        )
        gradcam = torch.nn.ReLU()(
...
def act_feature(
    ...
    acts = torch.mean(acts, dim=0)
    acts -= acts.min(1, keepdim=True)[0]
    acts /= acts.max(1, keepdim=True)[0]
